...
client.start(config.music1.token)
...
{ // 슬래쉬 커맨드 핸들
    const rest = new REST({ version: '10' }).setToken(config.music1.token)
    const commands = []
...
{ // 코드실행 핸들러
    ...
    client.on(Events.MessageCreate, async (message) => {
        try {
            ...
            if (!message.content.startsWith(config.music1.prefix)) return
            const args = await message.content.slice(config.music1.prefix.length).trim().split(/ +/)
